// Admin Panel JavaScript - Version Complète et Fonctionnelle
...
async function handleLogin(e) {
    ...
    try {
        ...
        if (error.message === 'Timeout de connexion') {
            showLoginError('Timeout de connexion. Vérifiez votre connexion internet et réessayez.');
        } else if (error.code === 'auth/network-request-failed') {
            showLoginError('Erreur réseau. Vérifiez votre connexion internet.');
        } else if (error.code === 'auth/too-many-requests') {
            showLoginError('Trop de tentatives. Attendez quelques minutes avant de réessayer.');
        } else {
...
async function loadAnalytics() {
    ...
    try {
        ...
        createSalesChart(analyticsOrders);
...
function createSalesChart(orders) {
    ...
    last7Days.forEach((day, index) => {
        const barHeight = (day.orders / maxOrders) * barMaxHeight;
        ...
        ctx.fillText(day.date, x + (barWidth - 20) / 2, canvas.height - 5);
        
        // Nombre de commandes
        ctx.fillText(day.orders.toString(), x + (barWidth - 20) / 2, y - 5);
    });
}
...
// ===== CONFIGURATION DES EVENT LISTENERS POUR TOUS LES BOUTONS =====
